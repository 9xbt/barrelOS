...
struct heap *heap_create() {
    struct heap *h = (struct heap *)HIGHER_HALF(pmm_alloc(1));
    h->head = (struct heap_block *)HIGHER_HALF(pmm_alloc(1));
    h->head->next = h->head;
    h->head->prev = h->head;
    h->head->size = 0;
    h->head->magic = HEAP_MAGIC;

    printf("[%5d.%04d] %s:%d: created heap at address 0x%lx\n", 0, 0, __FILE__, __LINE__, (uint64_t)h);
    return h;
}
...
void heap_free(void *ptr) {
    ...
    if (block->magic != HEAP_MAGIC) {
        printf("[%5d.%04d] %s:%d: bad block magic at address 0x%lx\n", 0, 0, __FILE__, __LINE__, (uint64_t)block);
        return;
    }
